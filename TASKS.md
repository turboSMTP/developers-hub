...
| 1.5 | Commit finalized OpenAPI 3.1 spec (multi-file `api-reference/turbo-smtp.yaml` + `api-reference/Domains/*.yaml`, synced from `turbo-api-2`) | DONE | 0.5 | Depends on 1.4. Spec lives in `api-reference/` — a single folder holding the self-contained Swagger UI bundle plus the narrative `README.md` overview |
| 1.6 | Deploy Swagger UI to GitHub Pages | DONE | 1 | **Automated** by `deploy-swagger-ui.yml` (publishes `api-reference/` on push to `main`). Pages enabled (Source: GitHub Actions); live at https://turbosmtp.github.io/developers-hub/ |
...
| # | Task | Status | Weeks Effort | Notes |
|---|---|---|---|---|
| 2.1 | Write full `getting-started` documentation | DONE | 0.5 | Authored from OpenAPI spec (published to `main`) |
| 2.2 | Write full `transactional` documentation | DONE | 0.5 | Authored from OpenAPI spec (published to `main`) |
| 2.3 | Write full `validation` documentation | DONE | 0.5 | Authored from OpenAPI spec (published to `main`) |
| 2.4 | Write full `webhooks` documentation | DONE | 0.5 | Rewritten from official Event Webhook Reference (published to `main`) |
| 2.5 | Make repository public | DONE | 0.5 | Repo public; all gating docs (2.1–2.4, 2.6–2.9) and 1.5 complete. Content published to `main` and live on GitHub Pages |
| 2.6 | Write full `analytics` documentation | DONE | 0.5 | New page, authored from OpenAPI spec (published to `main`) |
| 2.7 | Write full `suppressions` documentation | DONE | 0.5 | New page, authored from OpenAPI spec (published to `main`) |
| 2.8 | Write full `account` documentation (consumer keys, alerts, billing, password, meta) | DONE | 0.5 | New page, authored from OpenAPI spec (published to `main`) |
| 2.9 | Write full `subaccounts` documentation | DONE | 1 | New page, authored from OpenAPI spec (published to `main`) |
